...
# 计算模块度
# 将节点划分为社区
com_dict = {node:com for node, com in zip(G.nodes(), sc_com)}
communities = [[] for _ in range(k)]
for node, community in com_dict.items():
...
# 可视化
pos = nx.spring_layout(G_graph, seed=4, k=0.40)
plt.figure(dpi=400)  # 高分辨率图像
